...
- resources 为只读 `/app/data`，必须有 manifest.json：`{"files":{"relative/path":{"size":123,"sha256":"<hex>"}}}`。启动做 full hash；周期探测仅存在/尺寸检查。独立可写 cache 挂 `/var/cache/stella`，避开只读资源卷内嵌套创建挂载点。两节点 bundle 与 signing-key 指纹一致才可计划。

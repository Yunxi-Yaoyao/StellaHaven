...
def list_iperf(user: User = Depends(current_user), db: Session = Depends(get_db)):
    return task_svc.list_iperf(db)


@router.get("/iperf-tasks/{task_id}", response_model=IperfTaskRead)
def get_iperf(task_id: int, user: User = Depends(current_user), db: Session = Depends(get_db)):
...
def iperf_progress(task_id: int, token: str, ts: str, bitrate: float,
